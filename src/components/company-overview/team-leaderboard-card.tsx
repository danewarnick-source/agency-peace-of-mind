        ...
        <ol className="space-y-2">
          {items.slice(0, 5).map((e, idx) => (
            <li key={e.userId} className="flex items-center gap-3 rounded-lg border border-border bg-background px-3 py-2">
              <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-[var(--hive-gold)]/15 text-xs font-bold text-[#7a4a0a]">
                {idx + 1}
              </span>
              ...
              <span className="shrink-0 font-display text-sm font-bold tabular-nums text-[var(--hive-text)]">{e.score}</span>
            </li>
          ))}
        </ol>
      )}
