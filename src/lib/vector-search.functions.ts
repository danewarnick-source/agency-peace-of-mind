...
function validateSearch(input: unknown): SearchInput {
  ...
  const hourMin =
    typeof i.hourMin === "number" && i.hourMin >= 0 && i.hourMin <= 23 ? Math.floor(i.hourMin) : null;
  const dateFrom = typeof i.dateFrom === "string" && i.dateFrom.length > 0 ? i.dateFrom : null;
  const dateTo = typeof i.dateTo === "string" && i.dateTo.length > 0 ? i.dateTo : null;
  const matchCount =
    typeof i.matchCount === "number" && i.matchCount > 0 && i.matchCount <= 200
      ? Math.floor(i.matchCount)
      : 50;
  return { query, organizationId, hourMin, dateFrom, dateTo, matchCount };
}
...
export const searchTimesheetsByVector = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // pgvector accepts the JSON-array string form for vector literals
    const vecLiteral = `[${vec.join(",")}]`;
    const { data: rows, error } = await context.supabase.rpc(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      "match_timesheets" as any,
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      {
        query_embedding: vecLiteral,
        hour_min: data.hourMin,
