...
test.describe("Records review — admin harness", () => {
  ...
  test("header CTAs: Utah CSV dialog opens (no submit), Master Ledger present, Agency file navigates", async ({
    page,
  }) => {
    await openDesk(page);
    ...
    await expect(page.getByRole("button", { name: /Agency file/i })).toBeVisible();
    ...
    await page.getByRole("button", { name: /Agency file/i }).click();
