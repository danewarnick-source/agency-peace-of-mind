...
function evidenceLabel(t: CompanyObligationRow["evidence_type"]): string {
  switch (t) {
    ...
    case "upload":
      return "Upload";
    case "upload_and_attestation":
      return "Upload + Attestation";
    case "form":
      return "Form";
    default:
      return t;
...
function InstanceStatusLine({ instance }: { instance: ObligationInstanceRow | null }) {
  ...
  if (instance.status === "waived") {
    return (
      <p className="text-sm font-medium text-muted-foreground">Waived — {instance.waive_reason}</p>
    );
  }
  if (instance.status === "overdue") {
    const days = Math.max(
      1,
      Math.floor((Date.now() - new Date(instance.due_at).getTime()) / 86_400_000),
    );
    return (
  ...
  return (
    <p className="text-sm font-medium text-warning-foreground">Due {formatDate(instance.due_at)}</p>
  );
}
...
function PerNameCompletion({
    ...
    queryFn: async () => {
      ...
      const [assignees, { data: completions, error: cErr }] = await Promise.all([
        ...
        supabase
          ...
          .select(
            "id, staff_id, staff_name, completed_at, nectar_validation_status, nectar_validation_reasons, nectar_extracted_cert_type",
          )
          .in("instance_id", instanceIds.length ? instanceIds : [instanceId as string]),
      ]);
      if (cErr) throw new Error(cErr.message);
      const dueByStaff = new Map<string, string>();
      for (const row of (instRows ?? []) as Array<{
  ...
  const isClosed =
    obligation.current_instance.status === "completed" ||
    obligation.current_instance.status === "waived";

  if (isClosed && !obligation.requires_individual_completion) {
    const by =
      obligation.current_instance.completed_by_name ?? completions[0]?.staff_name ?? "someone";
    return (
      <p className="text-xs text-muted-foreground">
        Satisfied by {by} — any one submission closes this obligation.
        {assignees.length > 0 && (
          <span className="block">
            All assigned: {assignees.map((a) => a.staff_name).join(", ")}
          </span>
        )}
      </p>
    );
    ...
    <div className="grid gap-1.5 text-xs sm:grid-cols-2">
          ...
          <ul className="space-y-0.5">
            {completions.map((c) => {
              ...
              return (
                <li
                  key={c.staff_id}
                  className={failed ? "text-amber-600 dark:text-amber-400" : "text-success"}
                >
                  <div className="flex items-center gap-1">
                    ...
                    <span className="truncate">
                      {c.staff_name} —{" "}
                      {failed ? "NECTAR could not verify" : formatDate(c.completed_at)}
                    </span>
                    {failed && instanceId && (
                      <ConfirmNectarOverrideButton
                        orgId={orgId}
                  ...
                  {failed && (c.nectar_validation_reasons?.length ?? 0) > 0 && (
                    <p className="ml-4 text-[11px] text-muted-foreground">
                      {c.nectar_validation_reasons!.join("; ")}
                    </p>
                  )}
                </li>
              );
      ...
      <div>
        ...
        {notSubmitted.length === 0 ? (
          ...
          <ul className="space-y-0.5">
            {notSubmitted.map((a) => {
              const { text, overdue } = dueStatusText(
                dueByStaff.get(a.staff_id) ?? obligation.current_instance?.due_at ?? null,
              );
              return (
                <li
                  key={a.staff_id}
  ...
  return (
    <div className="space-y-1.5">
      ...
      >
        ...
        <ChevronDown
          className={`h-3 w-3 shrink-0 transition-transform md:hidden ${expanded ? "rotate-180" : ""}`}
        />
      </button>
...
function PerClientCompletion({ obligation }: { obligation: ObligationWithInstance }) {
      ...
      const [{ data: assignees, error: aErr }, { data: completions, error: cErr }] =
        instanceIds.length
          ? await Promise.all([
              ...
              supabase
                .from("company_obligation_completions")
                .select("instance_id, staff_id, staff_name, completed_at")
                .in("instance_id", instanceIds),
            ])
          : [
              { data: [], error: null },
  ...
  if (!instances.length)
    return <p className="text-xs text-muted-foreground">No active staff+client assignments yet.</p>;
  ...
  const doneCount = instances.filter(
    (inst) => inst.status === "completed" || completionByInstance.has(inst.id),
  ).length;
    ...
    <div className="space-y-1.5">
      ...
      <ul className={`space-y-1 text-xs ${expanded ? "block" : "hidden"} md:block`}>
        {instances.map((inst) => {
          ...
          const { text, overdue } = done
            ? { text: formatDate(completion?.completed_at ?? inst.completed_at), overdue: false }
            : dueStatusText(inst.due_at);
          return (
            <li
              key={inst.id}
...
function LogEventDialog({
  ...
  const daysAfter = Number(
    (obligation.due_day_config as Record<string, unknown> | null)?.days_after_trigger ?? 0,
  );
...
export function ObligationCard({
  ...
  const toggleActive = useMutation({
    mutationFn: () =>
      toggleFn({
        data: { organizationId: orgId, obligationId: obligation.id, active: !obligation.active },
      }),
    onSuccess: () => {
  ...
  const formArchived =
    obligation.evidence_type === "form" && obligation.linked_form_id
    ...
    >
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              ...
              variant="ghost"
              size="icon"
              className="h-7 w-7 shrink-0 self-end md:self-auto"
            >
            ...
            {obligation.is_locked ? (
              ...
              <>
                ...
                {obligation.cadence === "per_event" && (
                  <DropdownMenuItem onSelect={() => setLogEventOpen(true)}>
                    Log event
                  </DropdownMenuItem>
                )}
                <DropdownMenuItem onSelect={() => setManualOpen(true)}>
      ...
      {formArchived ? (
        <div className="mt-3 flex items-start gap-2 rounded-md border border-destructive/30 bg-destructive/10 p-2.5 text-sm text-destructive">
          ...
          <span>
            Linked form has been archived. Edit this obligation to assign a different form.
          </span>
        </div>
      ) : (
        <>
          <div className="mt-3 flex flex-wrap items-center gap-1.5 text-sm">
            ...
            {obligation.scope === "org" ? (
              ...
              <span className="text-sm text-muted-foreground">
                All active staff+client assignments
              </span>
          ...
          <div className="mt-3">
            {obligation.rollup ? (
              <RollupStatus
                rollup={obligation.rollup}
                reminderOnly={catalogFor(obligation)?.calendar_is_reminder_only}
              />
            ) : (
